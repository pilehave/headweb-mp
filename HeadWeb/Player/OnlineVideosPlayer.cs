...
namespace HeadWeb
{
    public class OnlineVideosPlayer : VideoPlayerVMR9, OVSPLayer
    {
      ...
      public int playbuffer = 1;   // percent
        ...
        public override void Process()
        {
            if (PercentageBuffered < 100.0f && graphBuilder != null && (DateTime.Now - lastProgressCheck).TotalMilliseconds > 100)
            {
                ...
                try
                {
                    int result = graphBuilder.FindFilterByName(httpSourceFilterName, out sourceFilter);
                    if (result == 0)
                    {
                ...
                {
                    Log.Warn("Error Quering Progress: {0}", ex.Message);
                }
                finally
                {
        ...
        public bool? PrepareGraph()
        {
            ...
            string sourceFilterName = uri.Scheme == "http" ? httpSourceFilterName : (uri.Scheme == "mms" || CurrentFile.ToLower().Contains(".asf")) ? "WM ASF Reader" : string.Empty;
            if (!string.IsNullOrEmpty(sourceFilterName))
            {
                ...
                {
                    Log.Warn("Error adding '{0}' filter to graph: {1}", sourceFilterName, ex.Message);
                    return null;
                }
                finally
                {
        ...
        public bool BufferFile()
        {
            ...
            try
            {
                ...
                if (result != 0)
                {
                  Log.Warn("HeadWeb BufferFile : FindFilterByName returned {0}", result);
                  return false;
                }
                
                // translate url if usage of rtmp proxy is not wanted
                string urlToLoad = CurrentFile;
                if (uri.Scheme == "http" && !useRtmpProxy)
                {
                ...
                if (result != 0)
                {
                  Log.Warn("BufferFile : IFileSourceFilter.Load returned '{0}' ({1})", result, DirectShowLib.DsError.GetErrorText(result));
                  return false;
                }
                ...
                if (sourceFilter is IAMOpenProgress && !CurrentFile.Contains("live=true"))
                {
                    ...
                    do
                    {
                        ...
                        if (!filterConnected && (PercentageBuffered >= playbuffer || skipBuffering))
                        {
                            if (skipBuffering) Log.Debug("HeadWeb Buffering skipped at {0}%", PercentageBuffered);
                        ...
                        {
                            Log.Debug("HeadWeb Buffering: {0}/{1} KB ({2}%)", current / 1024, total / 1024, (int)PercentageBuffered);
                            last = current;
                        }
                        ...
                        GUIPropertyManager.SetProperty("#HeadWeb.buffered", PercentageBuffered.ToString(formatString, System.Globalization.CultureInfo.InvariantCulture));
            ...
            finally
            {
                if (sourceFilter != null)
                {
                    if (!PlaybackReady)
                    {
                        Log.Info("HeadWeb Buffering was aborted");
        ...
        bool FinishPreparedGraph()
        {
            try
            {
                ...
                {
                    Log.Error("HeadWeb Failed to render file -> vmr9");
                    mediaCtrl = null;
                    Cleanup();
                    return false;
                }
            ...
            catch (Exception ex)
            {
                ...
                Log.Error("HeadWeb exception while creating DShow graph {0} {1}", ex.Message, ex.StackTrace);
                return false;
            }
        }
        ...
        public override bool Play(string strFile)
        {
            ...
            _updateNeeded = true;
            Log.Info("HeadWeb Play {0}", strFile);
            ...
            if (!engine.LoadSubtitles(graphBuilder, HeadWebSettings.Instance.s_movietitle + ".mp4")) // Construct a fake moviefile
            {
            ...
            Log.Info("HeadWeb Duration:{0}", m_dDuration);
            AnalyseStreams();
#if !MP11
            SelectSubtitles();
            SelectAudioLanguage();
        ...
        {
            Log.Info("HeadWeb Ended {0}", m_strCurrentFile);
            m_strCurrentFile = "";
            CloseInterfaces();
            m_state = PlayState.Init;
