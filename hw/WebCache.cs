...
namespace HeadWeb
{
    public class WebCache
    {
        ...
        WebCache() 
        {
          ...
          if (HeadWebSettings.Instance.CacheTimeout > 0)
            {
        ...
        public string this[string url]
        {
            get 
            {
              if (HeadWebSettings.Instance.CacheTimeout > 0) // only use cache if a timeout > 0 was set
                {
                    lock (this)
                    {
        ...
        void CleanCache(object state)
        {
            lock (this)
            {
                ...
                foreach (string key in cache.Keys)
                  if ((DateTime.Now - cache[key].LastUpdated).TotalMinutes >= HeadWebSettings.Instance.CacheTimeout)
