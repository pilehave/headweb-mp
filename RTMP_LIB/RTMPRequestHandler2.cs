	...
	public class RTMPRequestHandler : CrossDomanSingletonBase<RTMPRequestHandler>, IRequestHandler
    {
        ...
        public void HandleRequest(string url, HTTPServerRequest request, HTTPServerResponse response)
        {
            ...
            try
            {
                ...
                Log.Debug("SetupUrl returned " + res);
                Log.Debug("Protocol : " + lnk.protocol);
                Log.Debug("Hostname : " + lnk.hostname);
                Log.Debug("Port     : " + lnk.port);
                Log.Debug("Playpath : " + lnk.playpath);
                if (!String.IsNullOrEmpty(lnk.tcUrl))
                  Log.Debug("tcUrl    : " + lnk.tcUrl);
                if (!String.IsNullOrEmpty(lnk.swfUrl))
                  Log.Debug("swfUrl   : " + lnk.swfUrl);
                if (!String.IsNullOrEmpty(lnk.pageUrl))
                  Log.Debug("pageUrl  : " + lnk.pageUrl);
                if (!String.IsNullOrEmpty(lnk.app))
                  Log.Debug("app      : " + lnk.app);
                if (!String.IsNullOrEmpty(lnk.auth))
                  Log.Debug("auth     : " + lnk.auth);
                if (!String.IsNullOrEmpty(lnk.subscribepath))
                  Log.Debug("subscribepath : " + lnk.subscribepath);
                if (!String.IsNullOrEmpty(lnk.usherToken))
                  Log.Debug("NetStream.Authenticate.UsherToken : " + lnk.usherToken);
                if (!String.IsNullOrEmpty(lnk.flashVer))
                  Log.Debug("flashVer : " + lnk.flashVer);
                /*if (dStart > 0)
                ...
                Log.Debug("timeout  : " + lnk.timeout + " sec");
                    ...
                    do
                    {
						...
						if (responseStream == null)
						{
							if (nread > 13) // first 13 bytes are the flv header, after that first packet should be the metadata packet
							{
								...
								if (request.Get("User-Agent") != HeadWeb.HeadWebSettings.Instance.UserAgent)
